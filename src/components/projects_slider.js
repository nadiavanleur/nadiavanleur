...
export default ({ projects, title, visual, icon }) => {
  return (
    <section className="projects-slider page__section" id="projects">
      <div className="wrapper-inner projects-slider__wrapper-inner">
        {title && (
          <h2 className="projects-slider__title page__title">{title}</h2>
        )}
        <SlickSlider elementClass="projects-slider__slider">
          {projects.map(({ node }) => {
        ...
        {visual && visual.sizes.src !== null && (
          <Img
            ...
            sizes={visual.sizes}
            className="projects-slider__visual visual"
          />
        )}
